...
def main(galaxy_name, galaxy_type, redshift, max_workers):
    ...
    write_file_path = f"{cloudy_gas_particles_file_directory}/otherProperties_averageSobolevH_hybridInterpolator_2.txt"
        ...
        for k in k_values: # For loop is legacy 
            if k < max(k_values):
                try:
                    ...
                    interpolated_Y_values_check = 10**interpolator(gas[gas_data_column_names])[0] # It returns an array of arrays. That's why [0] is done.

                    # Check if there is nan 
                    contains_nan = np.isnan(interpolated_Y_values_check).any()
                    if contains_nan:
                        continue
                    else: 
                        # Set used interpolator
                        break # Break the loop as it interpolate

                except Exception as e: # Unable to construct the linearNDInterpolator. Qhull precision error most likely
...
def write_to_a_file(write_file_path, train_data_file_paths, gas_column_names, properties_column_names, merged_df, number_of_significant_digits):
