...
def main(galaxy_name, galaxy_type, redshift, max_workers, write_interpolator_info=False):
    ...
    write_file_path = f"{cloudy_gas_particles_file_directory}/L_line_averageSobolevH_hybridInterpolator_flux2Luminosity_2.txt"
        ...
        for k in k_values: 
            if k < max(k_values):
                try:
                    ...
                    interpolated_intensities_check = 10**interpolator(gas[gas_data_column_names])[0] # It returns an array of arrays. That's why [0] is done.

                    # Check if there is nan 
                    contains_nan = np.isnan(interpolated_intensities_check).any()
                    if contains_nan:
                        continue
                    else: 
                        # Set used interpolator
                        used_interpolator = "LinearNDInterpolator"

                        break # Break the loop as it interpolated
